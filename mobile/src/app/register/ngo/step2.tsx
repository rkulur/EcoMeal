...
import { step2Schema } from "@/src/validation/ngo/ngoRegistration.schema";
...
const NgoRegistrationStep2 = () => {
  ...
  const { data, setData } = useStep2NgoData();
  const router = useRouter();

  const onSubmit = (data: step2Type) => {
    setData(data);
    alert("Step 2 completed");
    router.push("/register/ngo/step3");
  };
  return (
    <StepSkeleton totalSteps={4} currStep={2} heading="Where can we find you?">
      <Controller
        control={control}
...
export default NgoRegistrationStep2;
